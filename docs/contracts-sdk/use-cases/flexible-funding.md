---
sidebar_label: Flexible Funding (TechForge)
sidebar_position: 5
---
...
After the transaction is mined, resolve the deployed `CampaignInfo` address. Two approaches are available — prefer the receipt-based one when you have the receipt in hand.
...
let kwrTreasuryAddress: `0x${string}` | undefined;
...
for (const log of deployReceipt.logs) {
  try {
    ...
    if (decoded.eventName === TREASURY_FACTORY_EVENTS.TreasuryDeployed) {
      kwrTreasuryAddress = decoded.args?.treasuryAddress as `0x${string}`;
      break;
    }
  } catch { /* log from a different contract */ }
}

const kwrTreasury = oak.keepWhatsRaisedTreasury(kwrTreasuryAddress!);
```
...
<MermaidDiagram title="TechForge Keep-What's-Raised Campaign Flow">

```mermaid
sequenceDiagram
    participant Creator as Creator (Lena)
    participant TechForge as TechForge Platform
    participant Backers
    participant Treasury as KeepWhatsRaised Treasury

    rect rgb(40, 40, 60)
    Note over Creator,Treasury: Campaign setup
    Creator->>Treasury: createCampaign(...)<br/>[Any caller]
    Note over Treasury: Campaign created
    TechForge->>Treasury: deploy(hash, info, 1)
    Note over Treasury: KWR treasury deployed
    TechForge->>Treasury: configureTreasury(...)
    Note over Treasury: Delays, fees configured
    Creator->>Treasury: addRewards(...)<br/>[Campaign Owner]
    Note over Treasury: Reward tiers set
    end

    rect rgb(40, 60, 40)
    Note over Backers,Treasury: Pledge + tip
    Backers->>Treasury: pledgeForAReward()
    Note over Treasury: NFT minted, funds + tip locked
    TechForge->>Treasury: setPaymentGatewayFee()<br/>[Platform Admin]
    Note over Treasury: Gateway fee recorded
    end

    rect rgb(60, 60, 40)
    Note over Creator,Treasury: Mid-campaign withdrawal
    TechForge->>Treasury: approveWithdrawal()
    Note over Treasury: Withdrawal approved
    Creator->>Treasury: withdraw(token, amt)
    Note over Treasury: Partial funds to creator
    end

    rect rgb(60, 60, 40)
    Note over Creator,Treasury: After deadline
    TechForge->>Treasury: disburseFees()
    Note over Treasury: Fees distributed
    TechForge->>Treasury: claimTip()
    Note over Treasury: Tips to platform
    TechForge->>Treasury: claimFund()
    Note over Treasury: Remaining funds to creator
    end

    rect rgb(60, 40, 40)
    Note over Backers,Treasury: Backer refund (after delay)
    Backers->>Treasury: claimRefund(tokenId)
    Note over Treasury: Backer refunded, NFT burned
    end
```
