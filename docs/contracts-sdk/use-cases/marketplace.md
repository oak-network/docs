---
sidebar_label: Marketplace (CeloMarket)
sidebar_position: 4
---
...
<MermaidDiagram title="CeloMarket E-Commerce Order Flow">

```mermaid
sequenceDiagram
    participant Buyer as Buyer (Alex)
    participant CeloMarket as CeloMarket (Platform Admin)
    participant Treasury as PaymentTreasury

    Buyer->>CeloMarket: Browse & order

    rect rgb(40, 40, 60)
    Note over Buyer,Treasury: Flow A — Off-chain / fiat payment
    CeloMarket->>Treasury: createPayment()<br/>[Platform Admin]
    Note over Treasury: Order recorded<br/>(no pull from buyer)
    Buyer->>CeloMarket: Buyer pays off-chain<br/>(credit card, etc.)
    CeloMarket->>Treasury: Tokens to treasury<br/>(deposit / bridge / ops)
    Note over Treasury: Balance must cover confirm
    Note over CeloMarket: Seller ships product
    CeloMarket->>Treasury: confirmPayment()<br/>[Platform Admin]
    Note over Treasury: Flow A: pending → confirmed
    end

    rect rgb(40, 60, 40)
    Note over Buyer,Treasury: Flow B — On-chain crypto payment
    Buyer->>Treasury: ERC-20 approve()
    Note over Treasury: Treasury approved
    CeloMarket->>Treasury: processCryptoPayment()<br/>[Any caller]
    Note over Treasury: Pull + confirmed + NFT
    end

    rect rgb(60, 60, 40)
    Note over Buyer,Treasury: Both flows (after Flow A confirm or Flow B)
    CeloMarket->>Treasury: claimNonGoalLineItems()<br/>[Platform Admin]
    Note over Treasury: Non-goal items claimed
    CeloMarket->>Treasury: disburseFees()<br/>[Any caller]
    Note over Treasury: Fees → Protocol + Platform
    CeloMarket->>Treasury: withdraw()<br/>[Admin or Owner]
    Note over Treasury: Funds → Seller
    end

    rect rgb(60, 40, 40)
    Note over Buyer,Treasury: Pause / Cancel (optional)
    CeloMarket->>Treasury: pauseTreasury() / cancelTreasury()<br/>[Platform Admin / Owner]
